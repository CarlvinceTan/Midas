...
import { readSessionLines, type SessionLine } from './fs-utils.ts'
...
async function resolveSession(arg: string | undefined, provider: 'claude' | 'codex'): Promise<SessionRef | null> {
  ...
  if (provider === 'codex') {
    const codex = await import('./context-tree-codex.ts')
...
export async function runContextCommand(
  ...
  if (opts.list) {
    const refs =
      provider === 'codex' ? await (await import('./context-tree-codex.ts')).listRecentCodexSessions(15) : await listRecentTitledSessions(15)
    if (refs.length === 0) {
  ...
  const result =
    provider === 'codex' ? await (await import('./context-tree-codex.ts')).buildCodexContextTree(session) : await buildContextTree(session)
  if (opts.json) {
