...
import { checkCliCompat, parsePluginManifest, type PluginManifest } from './manifest.ts'
import { RELEASE_PUBLIC_KEYS } from './keys.ts'
...
const { version: PKG_VERSION } = ((): { version: string } => {
  ...
  for (const candidate of ['../package.json', '../../package.json', '../../../package.json']) {
    try {
      ...
      // Vendored into midas; try the next layout.
    }
  }
