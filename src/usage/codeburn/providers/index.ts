...
import { clineCli } from './cline-cli.ts'
import { codewhale } from './codewhale.ts'
...
import { ibmBob } from './ibm-bob.ts'
import { kiloCode } from './kilo-code.ts'
import { kiro } from './kiro.ts'
import { kimi } from './kimi.ts'
import { kimicode } from './kimicode.ts'
import { lingtaiTui } from './lingtai-tui.ts'
import { mistralVibe } from './mistral-vibe.ts'
import { mux } from './mux.ts'
import { openclaw } from './openclaw.ts'
import { openclaude } from './openclaude.ts'
import { openDesign } from './open-design.ts'
import { pi, omp } from './pi.ts'
import { qwen } from './qwen.ts'
import { quickdesk } from './quickdesk.ts'
import { rooCode } from './roo-code.ts'
import { zerostack } from './zerostack.ts'
import { grok } from './grok.ts'
import type { Provider, SessionSource } from './types.ts'
...
async function loadForge(): Promise<Provider | null> {
  ...
  try {
    const { forge } = await import('./forge.ts')
    forgeProvider = forge
    return forge
  } catch {
    return null
...
async function loadGoose(): Promise<Provider | null> {
  ...
  try {
    const { goose } = await import('./goose.ts')
    gooseProvider = goose
    return goose
  } catch {
    return null
...
async function loadCursor(): Promise<Provider | null> {
  ...
  try {
    const { cursor } = await import('./cursor.ts')
...
async function loadVercelGateway(): Promise<Provider | null> {
  ...
  try {
    const { vercelGateway } = await import('./vercel-gateway.ts')
...
async function loadCursorAgent(): Promise<Provider | null> {
  ...
  try {
    const { cursor_agent } = await import('./cursor-agent.ts')
...
async function loadCrush(): Promise<Provider | null> {
  ...
  try {
    const { crush } = await import('./crush.ts')
    crushProvider = crush
    return crush
  } catch {
    return null
...
async function loadZcode(): Promise<Provider | null> {
  ...
  try {
    const { zcode } = await import('./zcode.ts')
    zcodeProvider = zcode
    return zcode
  } catch {
    return null
