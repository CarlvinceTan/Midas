...
import { CATEGORY_LABELS, type ProjectSummary, type SessionSummary, type TaskCategory, type DateRange } from './types.ts'
import { isBehavioralCall } from './behavioral-weight.ts'
import { type PeriodData, type ProviderCost, type BreakdownArrays, type MenubarPayload, type ClaudeConfigSelector, type HydrationState, buildMenubarPayload } from './menubar-json.ts'
import { type SessionCountBasis } from './session-count-label.ts'
import { parseAllSessions, filterProjectsByName, filterProjectsByDays, filterProjectsByClaudeConfigSource, filterProjectsByDateRange, isSessionHydrationComplete, makeProjectFilter, type ProjectFilterTarget, sessionHydrationSnapshot } from './parser.ts'
type ProjectFilter = (entry: ProjectFilterTarget) => boolean

import { findUnpricedModels, getFlatRateModelsConfigHash, getLocalModelSavingsConfigHash, getPriceOverridesConfigHash, getShortModelName, isExpectedFreeModel } from './models.ts'
import { getAllProviders, safeDiscoverSessions } from './providers/index.ts'
import { loadPlugins, pluginPayloadSections } from './plugins/loader.ts'
import { collectLiveSessions } from './live-sessions.ts'
import { claude, getClaudeConfigDirs, getDesktopSessionsDirs } from './providers/claude.ts'
import { stat } from 'node:fs/promises'
import { aggregateProjectsIntoDays, buildPeriodDataFromDays, dateKeyInTz } from './day-aggregator.ts'
import { aggregateModelEfficiency } from './model-efficiency.ts'
import { aggregateModels } from './models-report.ts'
import { aggregateModelTaskTurns, sessionDurationMinutes } from './telemetry-snapshot.ts'
import { scanUserCorrections, medianTimeToFirstEditMs, aggregateFileChurn, computePricingCoverage } from './workflow-insights.ts'
import { buildPrAttribution, aggregateByBranch } from './sessions-report.ts'
import { scanAndDetect } from './optimize.ts'
import { callBillableOutputTokens, sessionBillableOutputTokens, inferSessionProvider } from './session-output.ts'
import { getDaysInRange, ensureCacheHydrated, loadDailyCache, cachedProjectIdentities, emptyCache, mergeDayEntries, BACKFILL_DAYS, toDateString, type DailyCache, type DailyEntry, type ProjectDayStats, type ProviderDaySlice } from './daily-cache.ts'
import { buildGranularHistory } from './granular-history.ts'
import { spendProjectIdentity } from './spend-flow.ts'
...
export function addProviderSlice(totals: Record<string, ProviderSliceTotal>, name: string, slice: ProviderDaySlice): void {
  ...
  if (slice.cacheReadTokens !== undefined) total.cacheReadTokens = (total.cacheReadTokens ?? 0) + slice.cacheReadTokens
  if (slice.cacheWriteTokens !== undefined) total.cacheWriteTokens = (total.cacheWriteTokens ?? 0) + slice.cacheWriteTokens
