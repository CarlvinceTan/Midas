...
test("UserPromptCard renders real image chips in yellow", () => {
  const card = new UserPromptCard("look at [Image: shot.png] please", 2, (text) => text, ["shot.png"]);
...
test("UserPromptCard leaves typed image-like text plain", () => {
  const card = new UserPromptCard("an example is [Image: ...] like so", 2, (text) => text, []);
  const rendered = card.render(80).join("\n");
  assert.ok(!rendered.includes("\x1b[33m"), "typed [Image: …] text must not be highlighted");
  assert.ok(stripAnsi(rendered).includes("[Image: ...]"));
});
...
test("UserPromptCard matches macOS screenshot names with narrow spaces", () => {
  ...
  const card = new UserPromptCard(`[Image: ${filename}]`, 2, (text) => text, [filename]);
  assert.ok(card.render(80).join("\n").includes("\x1b[33m"), "narrow no-break spaces are normalized before matching");
});
