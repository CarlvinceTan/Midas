...
/** Shared rounded frame row; middle is already rendered at the inner width. */
export function roundedFrameRow(middle, edge, color) {
    if (edge === "body") return color("│") + middle + "\x1b[0m" + color("│");
    const corners = edge === "top" ? ["╭", "╮"] : ["╰", "╯"];
    return "\x1b]777;pi-decoration\x07\x1b]777;pi-content-start\x07\x1b]777;pi-content-end\x07" + color(corners[0]) + middle + color(corners[1]);
}
