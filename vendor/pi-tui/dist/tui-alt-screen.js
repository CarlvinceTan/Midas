...
// Copy feedback: green background for success, red for failure.
const COPY_OK_STYLE = "\x1b[42m\x1b[30m";
const COPY_FAIL_STYLE = "\x1b[41m\x1b[97m";
...
export class TuiAltScreen extends TuiBase {
    ...
    flash(message, durationMs, style) {
        this.flashes.flash(message, durationMs, style);
    }
    ...
    getSelectionSourceLine(point) {
        ...
        return this.selectionScreen[point.row] ?? "";
    }
    ...
    getSelectionColumns(line, row, selection, minColumn = 0, maxColumn = visibleWidth(line)) {
        ...
        // Intersect selection with renderer-provided content bounds. Unlike
        // trimStart(), this preserves indentation that belongs to the content.
        let contentStart = minColumn;
        let contentEnd = Math.min(maxColumn, visibleWidth(stripTerminalSequences(line).trimEnd()));
        const markers = /\]777;pi-content-(start|end)/g;
        for (const match of line.matchAll(markers)) {
            const col = visibleWidth(line.slice(0, match.index));
            if (match[1] === "start") contentStart = Math.max(contentStart, col);
            else contentEnd = Math.min(contentEnd, col);
        }
        return { start: Math.max(minColumn, start, contentStart), end: Math.min(maxColumn, end, contentEnd) };
    }
    getActiveSelectionText() {
        const selection = this.getSelectionBounds();
        if (!selection)
            return undefined;
        let sourceLines = this.selectionScreen;
        ...
        for (let row = selection.start.row; row <= selection.end.row; row++) {
            ...
            if (line.includes("\x1b]777;pi-decoration\x07")) continue;
    ...
    async copyTextToClipboard(text) {
        ...
        if (this.copySelection) {
            ...
            this.flash(ok ? "Copied!" : "Copy failed", undefined, ok ? COPY_OK_STYLE : COPY_FAIL_STYLE);
            return ok;
        }
        this.terminal.write(`\x1b]52;c;${Buffer.from(text).toString("base64")}\x07`);
        this.flash("Copied!", undefined, COPY_OK_STYLE);
        return true;
    }
    ...
    doRender() {
        ...
        // Keep content markers intact: highlight slicing replays zero-width
        // markers, so the painted screen is not a safe source for copying.
        this.selectionScreen = screen.slice();
